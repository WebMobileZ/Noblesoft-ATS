...
@Component({
  selector: 'app-prime-vendors-list',
  templateUrl: './prime-vendors.component.html',
  styleUrls: ['./prime-vendors.component.scss']
})
export class PrimeVendorsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.consultantService.getExportPrimeVendors().then(data => this.selectedProducts = data);
...
getVendors()
{
  ...
  updateConsultantDetails() {
    ...
    if (this.primevendor.vendor_company_id) {
      this.consultantService.updatePrimeVendor(this.clientsForm, this.primevendor.vendor_company_id).subscribe(data => {
      ...
      this.consultantService.storePrimeVendor(this.clientsForm).subscribe(data => {
